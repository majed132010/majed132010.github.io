...
auth.getRedirectResult().then(result => {
  if (result && result.user) history.replaceState(null, '', location.href);
}).catch(e => {
  if (e.code && e.code !== 'auth/no-auth-event') console.error('redirect error:', e);
});
...
function signInGoogle() {
  ...
  auth.signInWithPopup(provider)
    ...
    .catch(e => {
      ...
      if (e.code === 'auth/popup-blocked') toast('⚠️ اضغط مجدداً لفتح تسجيل الدخول');
      else if (e.code !== 'auth/popup-closed-by-user' && e.code !== 'auth/cancelled-popup-request') toast('❌ خطأ: ' + (e.message || e.code));
    });
}
