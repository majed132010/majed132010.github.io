...
const _registeredServers = new Set();
let _serversListenerRef = null;

// ════ انتظار اكتمال التحميل الأول ════
...
// ════ استماع للسيرفرات — ✅ محسّن: لا يعيد تحميل إلا الجديد ════
function listenServers() {
 ...
 // ✅ أول تحميل: نحمل فقط السيرفرات غير الموجودة
 ref.once('value').then(snap => {
 const sids = snap.val() || {};
 const newSids = Object.keys(sids).filter(sid => !servers[sid]);
 if (newSids.length === 0) {
 if (!window._loaded) { window._loaded = true; restoreLastServer(); if (typeof listenDMs === 'function') listenDMs(); }
 return;
 }
 Promise.all(newSids.map(sid =>
...
function _makeSvIcon(sid, sv) {
 ...
 if (sv.avatarUrl) div.innerHTML = `<img src="${escHtml(sv.avatarUrl)}" style="width:100%;height:100%;object-fit:cover">`; else div.textContent = sv.emoji || sv.name?.[0] || '?';
 div.title = sv.name || '';
...
function renderHomeServers() {
 ...
 info.innerHTML = `<div style="font-weight:700;font-size:15px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escHtml(sv.name||'عالم')}</div><div style="font-size:12px;color:var(--muted)">${sorted.length} قناة</div>`;
 ...
 row.innerHTML = `${ch.type==='voice'?'🔊':'#'}${escHtml(ch.name)}`;
...
function renderChannels(sid) {
 ...
 el.dataset.cid = cid;
 el.innerHTML = `${icon}${escHtml(ch.name)}`;
 ...
 dmBtn.innerHTML = `💬رسائلي الخاصة${dmUnreadTotal>0?`<span class="ch-unread-badge">${dmUnreadTotal>99?'99+':dmUnreadTotal}</span>`:''}`;
...
async function openMemberMgmt() {
 ...
 container.innerHTML = '<div style="text-align:center;color:var(--muted);padding:20px;font-family:Tajawal,sans-serif">⏳ جاري التحميل...</div>';
 ...
 info.innerHTML = `<div style="font-weight:700;font-size:14px;color:#fff">${escHtml(name)}</div><div style="font-size:12px;color:var(--muted)">${roleLabel}</div>`;
...
async function _ensureMemberRegistered(sid) {
 ...
 if (_registeredServers.has(sid)) return;
 const authUser = auth.currentUser;
 if (!authUser) return;
 const displayName = userProfile.displayName || authUser.displayName || authUser.email?.split('@')[0] || 'عضو';
 ...
 _registeredServers.add(sid);
}

// ════ استماع عضوية السيرفر ════
