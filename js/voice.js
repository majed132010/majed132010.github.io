...
// ════ الحصول على Token مع إعادة المحاولة ════
async function getAgoraToken(channelName, uid, retries = 3) {
  for (let i = 1; i <= retries; i++) {
    try {
      if (i > 1) toast(`⏳ إعادة المحاولة ${i}...`);
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 20000); // زيادة الـ timeout
      const res = await fetch(`${TOKEN_SERVER_URL}/token?channel=${encodeURIComponent(channelName)}&uid=${uid}`, {
...
function showVoiceChannel(sid, cid, ch) {
  ...
  const nameEl = document.getElementById('voiceViewName');
...
async function selectMicrophone() {
  try {
    ...
    if (!devices || devices.length === 0) { toast('❌ لا يوجد ميكروفون متاح'); return; }
    if (devices.length === 1) { selectedMicDeviceId = devices[0].deviceId; toast('🎤 ' + (devices[0].label || 'الميكروفون الافتراضي')); return; }
    const names = devices.map((d,i) => `${i+1}. ${d.label||'ميكروفون '+(i+1)}`).join('\n');
    const choice = prompt('اختر رقم الميكروفون:\n' + names);
    const idx = parseInt(choice) - 1;
    if (idx >= 0 && idx < devices.length) { selectedMicDeviceId = devices[idx].deviceId; toast('✅ تم اختيار: ' + devices[idx].label); }
  } catch(e) { toast('❌ لا يمكن الوصول للميكروفون: ' + (e.message||'')); }
}
  ...
  try {
    ...
    _speakingDetector = setInterval(() => {
      ...
      if (isSpeaking !== wasSpeaking) {
        ...
        if (voiceChannel) db.ref('voice/' + voiceChannel.sid + '/' + voiceChannel.cid + '/' + currentUser.uid + '/speaking').set(isSpeaking).catch(()=>{});
      }
    }, 100);
...
function stopSpeakingDetection() {
  ...
  if (voiceChannel && currentUser) db.ref('voice/' + voiceChannel.sid + '/' + voiceChannel.cid + '/' + currentUser.uid + '/speaking').set(false).catch(()=>{});
}
...
// ════ الانضمام للقناة الصوتية — مُصلح ════
async function joinVoice() {
  if (!voiceChannel || !agoraClient) return;
  toast('⏳ جاري الاتصال بسيرفر الصوت...');
  ...
  try {
    ...
    if (!token) {
      toast('❌ فشل الحصول على تصريح الاتصال — تحقق من اتصالك بالإنترنت');
      if (localAudioTrack) { localAudioTrack.close(); localAudioTrack = null; }
      return;
    }

    // إعادة ربط الأحداث (تجنب التكرار)
    ...
    await db.ref('voice/' + voiceChannel.sid + '/' + voiceChannel.cid + '/' + currentUser.uid).set({
      name: userProfile.displayName || 'مستخدم', muted: false, speaking: false, ts: Date.now()
    });
    db.ref('voice/' + voiceChannel.sid + '/' + voiceChannel.cid + '/' + currentUser.uid).onDisconnect().remove();
    ...
    // إعادة إنشاء الـ client بعد الفشل
    agoraClient = AgoraRTC.createClient({ mode: 'rtc', codec: 'vp8' });
    _bindAgoraEvents();
  }
}
...
async function leaveVoice() {
  ...
  try {
    ...
    if (agoraClient) { agoraClient.removeAllListeners(); await agoraClient.leave(); }
...
async function toggleMute() {
  ...
  if (voiceChannel) await db.ref('voice/' + voiceChannel.sid + '/' + voiceChannel.cid + '/' + currentUser.uid + '/muted').set(isMuted);
...
// ════ رسائل صوتية ════
let _mediaRecorder = null, _audioChunks = [], _recordingTimer = null, _recordingSeconds = 0, _voiceRecordingBusy = false;
...
function buildVoiceMsg(url, duration) {
  ...
  dur.className = 'voice-duration'; dur.textContent = duration ? `${duration}s` : '0s';
  audio.ontimeupdate = () => {
    fill.style.width = (audio.duration ? audio.currentTime / audio.duration * 100 : 0) + '%';
    dur.textContent = Math.ceil(audio.duration - audio.currentTime) + 's';
  };
