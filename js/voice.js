...
// ✅ FIX: AudioContext واحد مشترك
let _sharedAudioCtx = null;
let _sharedAudioSrc = null;
let _sharedAudioAnalyser = null;
...
function showVoiceChannel(sid, cid, ch) {
 ...
 ul.innerHTML = '<div style="text-align:center;color:var(--muted);padding:20px;font-family:Tajawal,sans-serif">لا يوجد أحد في القناة</div>';
 return;
 }
 ...
 div.innerHTML = `
 <div class="voice-user-av" id="va-${uid}" style="border:2px solid ${isSpeaking?'#23a55a':'transparent'}">${(u.name||'?')[0]}</div>
 <div class="voice-user-info">
 <div class="voice-user-name">${escHtml(u.name||'مستخدم')}</div>
 <div class="voice-user-status" id="vi-${uid}">${u.muted ? '🔇 مكتوم' : isSpeaking ? '🎙️ يتحدث' : '🔊 صامت'}</div>
 </div>
 `;
 ul.appendChild(div);
 });
...
function startSpeakingDetection(localTrack) {
 ...
 const track = localTrack.getMediaStreamTrack();
 if (!_sharedAudioSrc) {
 _sharedAudioSrc = _sharedAudioCtx.createMediaStreamSource(new MediaStream([track]));
 } else {
 try { _sharedAudioSrc.disconnect(); } catch(e) {}
 _sharedAudioSrc = _sharedAudioCtx.createMediaStreamSource(new MediaStream([track]));
 }

 if (!_sharedAudioAnalyser) {
 _sharedAudioAnalyser = _sharedAudioCtx.createAnalyser();
 _sharedAudioAnalyser.fftSize = 512;
 }
 _sharedAudioSrc.connect(_sharedAudioAnalyser);

 const data = new Uint8Array(_sharedAudioAnalyser.frequencyBinCount);
 let wasSpeaking = false;
 _speakingDetector = setInterval(() => {
 _sharedAudioAnalyser.getByteFrequencyData(data);
...
function stopSpeakingDetection() {
 ...
 // ✅ إغلاق AudioContext المشترك عند المغادرة
 if (_sharedAudioCtx && _sharedAudioCtx.state !== 'closed') {
 try {
 if (_sharedAudioSrc) { _sharedAudioSrc.disconnect(); _sharedAudioSrc = null; }
 _sharedAudioCtx.close();
 } catch(e) {}
 _sharedAudioCtx = null;
 _sharedAudioAnalyser = null;
 }
...
let _recordingMaxTimer = null; // ✅ مؤقت الحد الأقصى
...
async function toggleVoiceRecording() {
 ...
 clearTimeout(_recordingMaxTimer); // ✅ إلغاء مؤقت الحد الأقصى
 _recordingMaxTimer = null;
 ...
 // ✅ FIX: إعلام المستخدم قبل التوقف التلقائي بعد 55 ثانية
 _recordingMaxTimer = setTimeout(() => {
