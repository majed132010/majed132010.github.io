...
function buildMsgDiv(msg, key) {
  ...
  if (msg.replyTo) {
    ...
    quote.innerHTML = `
      <div class="rq-name">${escHtml(msg.replyTo.name||'')}</div>
      <div class="rq-text">${escHtml(msg.replyTo.text||'🖼️ وسائط')}</div>
    `;
...
async function sendMessage() {
  ...
  const msgBase = {
    ...
    ts: Date.now(),
    role,
    isSnap: snapMode || false,
    expiresAt: snapMode ? Date.now() + 24 * 60 * 60 * 1000 : null,
    saved: false
  };
...
function renderReactions(reactions, msgKey, container) {
  ...
  Object.entries(reactions).forEach(([emoji, users]) => {
    ...
    chip.innerHTML = `${emoji}<span class="rc-count">${uids.length}</span>`;
...
function openLightbox(url, type, name, msgKey, canDelete, senderName, ts) {
  ...
  if (nameEl) nameEl.textContent = senderName || name || '';
  if (timeEl && ts) {
    const d = new Date(ts);
    timeEl.textContent = d.toLocaleString('ar-SA', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' });
  } else if (timeEl) timeEl.textContent = '';
  if (delBtn) delBtn.style.display = (canDelete && msgKey) ? 'flex' : 'none';
...
function _attachContextBar(div, body, actions, isMine) {
  ...
  actions.forEach(ac => {
    ...
    b.title = ac.label;
    b.innerHTML = `${ac.icon}<span class="mc-label">${ac.label}</span>`;
